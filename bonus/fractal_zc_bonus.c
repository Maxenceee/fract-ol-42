...
/*   Updated: 2023/04/30 00:19:16 by mgama            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_complex_number point_offset, t_data *mlx)
{
	...
	{
		temp_num = num;
		// num = complex_div(complex_sub(complex_rational_pow(temp_num, 3), point_offset), temp_num); // ok
		num = complex_add(complex_cos(temp_num), complex_div(create_complex_number(1, 0), point_offset)); // ok
		// num = complex_absv(complex_add(complex_rational_pow(temp_num, 2), complex_sin(complex_rational_pow(point_offset, 3)))); // ok
		// num = complex_cos(complex_div(temp_num, point_offset)); // ok
		// num = complex_add(complex_sinh(temp_num), complex_div(create_complex_number(1, 0), complex_rational_pow(point_offset, 2)));
		i++;
	}
