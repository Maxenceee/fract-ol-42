...
/*   Updated: 2022/11/27 23:31:55 by mgama            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_circle(t_circle circle, t_data *mlx)
{
	...
	while (i < 360)
	{
		...
		if (circle.center.x + x1 <= WINDOW_WIDTH && circle.center.x + x1 >= 0 &&
			circle.center.y + y1 <= WINDOW_WIDTH && circle.center.y + y1 >= 0)
			my_mlx_pixel_put(mlx, circle.center.x + x1, circle.center.y + y1, 0xFFFFFF);
		i += 0.1;
	}
}
...
{
	t_circle	c4 = get_adjacent(crls[0], crls[1], crls[2]);
	t_circle	c5 = flip_circle(crls[0], crls[1], crls[2], c4);
	...
	recursive_circle(crls[0], crls[1], crls[2], c4, mlx, 0);
	recursive_circle(c5, crls[1], crls[2], c4, mlx, 0);
	free(crls);
}
